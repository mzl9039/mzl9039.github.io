...
<div class="mermaid">
graph TB    
    executorBackend-->|远程事件RegisterExecutor|schedulerBackend    
    schedulerBackend-->|远程事件RegisteredExecutor|executorBackend
    launchTasks[launchTasks]-->|13.触发远程事件LaunchTask|executorBackend
    StatusUpdate[事件StatusUpdate]-->|24.调用对象taskScheduler|taskScheduler{taskScheduler}
    subgraph executor 端流程        
    executorBackend-->|14.执行事件LaunchTask|LaunchTask[LaunchTask]
    LaunchTask-->|15.调用对象executor|executor{executor}
    executor-->|16.执行方法launchTask|launchTask[launchTask]
    launchTask-->|17.生成对象TaskRunner|TaskRunner{TaskRunner}
    TaskRunner-->|18.执行方法run|run1[TaskRunner的run]
    run1-->|19.调用对象task|Task{Task}
    Task-->|20.执行方法run|run2{Task的run}
    run2-->|21.调用对象executorBackend|executorBackend
    executorBackend-->|22.执行方法statusUpdate|statusUpdate[statusUpdate]
    statusUpdate-->|23.触发远程事件StatusUpdate|StatusUpdate[事件StatusUpdate]    
    end
    subgraph driver 端流程
 ...
    hasLaunchedTask-->|5.为true，调用对象schedulerBackend|schedulerBackend
    schedulerBackend-->|6.执行方法reviveOffers|reviveOffers[reviveOffers]
    hasLaunchedTask-->|5.为false,循环等待|hasLaunchedTask  
    resourceOffers-->|5.设为true|hasLaunchedTask   
    reviveOffers-->|7.执行方法makeOffers|makeOffers[makeOffers]
    makeOffers-->|8.触发事件ReviveOffers|ReviveOffers[事件ReviveOffers]
    ReviveOffers-->|9.调用方法launchTasks|launchTasks[launchTasks]    
    launchTasks-->|10.调用对象schedulerBackend|schedulerBackend
    schedulerBackend-->|11.调用方法resourceOffers|resourceOffers[resourceOffers]
    resourceOffers-->|12.返回结果到launchTasks|launchTasks
    schedulerBackend-->|StatusUpdate|taskScheduler
    taskScheduler-->|25.执行方法statusUpdate，获取结果|taskResultGetter
    end    
</div>
