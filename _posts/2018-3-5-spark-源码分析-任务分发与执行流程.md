...
graph TD    
 ...
    new_scheduler-->initialize[scheduler初始化backend]
    new_backend[实例化StandaloneSchedulerBackend]-->initialize[scheduler初始化backend]
</div>

提交 Task 流程
<div class="mermaid">
graph TB
    launchTasks[launchTasks]-->|14.触发远程事件LaunchTask|executorBackend(executorBackend)
 ...
    resourceOffers-->|13.返回结果到launchTasks|launchTasks[launchTasks]    
    taskScheduler(taskScheduler)-->|26.执行方法statusUpdate,获取结果|taskResultGetter
    end
</div>
