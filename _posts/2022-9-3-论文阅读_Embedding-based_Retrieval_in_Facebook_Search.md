...
对于给定的 triplet: $loss=(q^{(i)}, d^{(i)}_{+}, d^{(i)}_{-})$，其中 $q^{(i)}$ 为 query, $d^{(i)}_{+}$ 和 $d^{(i)}_{-}$ 是相应的正负样本文档, 则 triplet loss 定义为:
