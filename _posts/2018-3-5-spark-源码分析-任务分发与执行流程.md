...
提交 Task 流程
<div class="mermaid" style="background:#ffffff;font-size:100%">
graph TB    
    subgraph executor 端流程    
    executorBackend-->|6远程事件RegisterExecutor|schedulerBackend    
    executorBackend-->LaunchTask[执行事件LaunchTask]
    LaunchTask-->executor[调用executor]
 ...
    subgraph driver 端流程
    dagScheduler-->|1执行submitMissingTasks|taskScheduler[调用taskScheduler]
    click dagScheduler "dagScheduler"
    taskScheduler-->|2执行submitTasks|hasLaunchedTask{hasLaunchedTask}
    hasLaunchedTask-->|3为true|schedulerBackend[调用schedulerBackend]
    hasLaunchedTask-->|4为false,循环等待|hasLaunchedTask    
    schedulerBackend-->makeOffers
    makeOffers-->launchTasks
    taskScheduler-->launchTasks
    launchTasks-->resourceOffers
    resourceOffers-->|设为true|hasLaunchedTask    
    schedulerBackend-->|StatusUpdate|taskScheduler
    taskScheduler-->|statusUpdate|taskResultGetter
    end    
    schedulerBackend-->|5远程事件RegisteredExecutor|executorBackend
    launchTasks-->|远程事件LaunchTask|executorBackend
</div>
