...
<div class="mermaid">
graph TD
    subgraph initialize TaskScheduler and SchedulerBackend
    ...
    master_url[spark.master]-->local
    master_url[spark.master]-->local_n
    master_url[spark.master]-->local_n_m
    master_url[spark.master]-->spark_url
    master_url[spark.master]-->local_cluster
    master_url[spark.master]-->other
    spark_url[spark://...]-->new_scheduler[schduler = new TaskSchedulerImpl(sc)]
    spark_url[spark://...]-->new_backend[backend = new StandaloneSchedulerBackend(scheduler, sc, masterUrls)]
    new_taskSchedulerImple[new TaskSchedulerImpl]-->initialize[scheduler.initialize(backend)]
    end
    subgraph submitTasks
    dagScheduler-->|submitMissingTasks|taskScheduler
    taskScheduler-->|submitTasks|hasLaunchedTask{hasLaunchedTask}
    hasLaunchedTask-->|true|schedulerBackend
    hasLaunchedTask-->|false|hasLaunchedTask
    executorBackend-->|event:RegisterExecutor|schedulerBackend
    schedulerBackend-->|event:RegisteredExecutor|executorBackend
    schedulerBackend-->|event:SparkListenerExecutorAdded|listenerBus
    schedulerBackend-->makeOffers
    makeOffers-->launchTasks
    taskScheduler-->|resourceOffers|launchTasks
    launchTasks-->|event:LaunchTask|executorBackend
    executorBackend-->LaunchTask
    LaunchTask-->executor
    executor-->|launchTask|TaskRunner
    TaskRunner-->|run|Task
    Task-->|statusUpdate|executorBackend
    executorBackend-->|event:StatusUpdate|schedulerBackend
    schedulerBackend-->|StatusUpdate|taskScheduler
    taskScheduler-->|statusUpdate|taskResultGetter
    end
</div>
