...
graph TD
    subgraph driver 端流程
    dagScheduler-->|(1)执行submitMissingTasks|taskScheduler[调用taskScheduler]
    taskScheduler-->|(2)执行submitTasks|hasLaunchedTask{hasLaunchedTask}
    hasLaunchedTask-->|(3)为true|schedulerBackend[调用schedulerBackend]
    hasLaunchedTask-->|(4)为false,循环等待|hasLaunchedTask
